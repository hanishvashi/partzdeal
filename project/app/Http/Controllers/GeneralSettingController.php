...
class GeneralSettingController extends Controller
{
    ...
    public function logoup(StoreValidationRequest $request)
    {
        ...
        $logo = Generalsetting::findOrFail(1);
			...
			if ($file = $request->file('mobile_logo'))
			{
				$name = time().$file->getClientOriginalName();
				$file->move('assets/images',$name);
				if($logo->mobile_logo != null)
				{
				if (file_exists(public_path().'/assets/images/'.$logo->mobile_logo)) {
				unlink(public_path().'/assets/images/'.$logo->mobile_logo);
				}
				}
				$input['mobile_logo'] = $name;
			}
