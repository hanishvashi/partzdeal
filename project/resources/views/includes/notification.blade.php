...
<?php if (Session::has('CURRENT_STORE_ID')){ ?>
  ...
  @php
  $store = \App\Stores::where('id',session('CURRENT_STORE_ID'))->first();
  @endphp

<div class="profile-store">
  <a id="product_store" data-toggle="modal" role="button" data-target="#chooseStoreModel">
  <span id="notf_product">{{$store->store_name}}</span>
  </a>
</div>
<?php } ?>
