                            ...
                            <div class="row">
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    ...
                                    @endif
											...
											<div class="form-group">
                                                <label class="control-label col-sm-4" for="admin_cover_photo">Cover Photo *</label>
                                                <div class="col-sm-2">
@if($user->is_provider == 1)
<img style="width: 100%; height: auto;" src="{{ $user->cover_photo ? $user->cover_photo:asset('assets/images/noimage.png')}}" alt="profile image">
@else
<img  style="width: 100%; height: auto;" id="admincoverimg" src="{{ $user->cover_photo ? asset('assets/images/'.$user->cover_photo):asset('assets/images/noimage.png')}}" alt="profile image">
@endif

                                                </div>
                                                @if($user->is_provider != 1)
                                                <div class="col-sm-4">
                                                    <input type="file" id="uploadCoverFile" class="hidden" name="cover_photo" value="">
                                                    <button  type="button" id="uploadCoverTrigger" onclick="uploadCoverclick()" class="btn btn-block add-product_btn adminImg-btn"><i class="fa fa-upload"></i> Change Photo</button>
                                                </div>
                                                @endif
                                            </div>
											...
											<div class="form-group">
                                                <label class="control-label col-sm-4" for="dash_state">State *</label>
                                                <div class="col-sm-6">
												<select class="form-control" required="" id="dash_state" name="state">
...
@section('scripts')
    <script type="text/javascript">
		...
		function uploadCoverclick(){
            $("#uploadCoverFile").click();
            $("#uploadCoverFile").change(function(event) {
                readCoverURL(this);
                $("#uploadCoverTrigger").html($("#uploadCoverFile").val());
            });
		...
		function readCoverURL(input) {
            if (input.files && input.files[0]) {
                ...
                reader.onload = function (e) {
                    $('#admincoverimg').attr('src', e.target.result);
                }
