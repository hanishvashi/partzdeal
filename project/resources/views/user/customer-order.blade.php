...
@section('content')
    <section class="customer-order-info">
        <div class="container">
            <div class="row">
                                        ...
                                        <div class="row reorder-xs">
                                            <div class="col-lg-8 col-md-5 col-sm-5 col-xs-12">
                                                <div class="product-header-title">
                                                    <h2>Order# {{$order->order_number}} [{{$order->status}}]  <a href="{{ route('customer-orders') }}" class="btn add-newProduct-btn" style="padding: 5px 12px;"  class="print-order-btn">
                                                        ...
                                                        <div class="billing-add-area">
                                                            <div class="row">
                                                                ...
                                                                <div class="col-md-6">
                                                                    ...
                                                                    <p>Paid Amount: {{$order->currency_sign}}{{ number_format($order->pay_amount * $order->currency_value , 2) }}</p>
                                                                    <p>Payment Method: {{$order->method}}</p>

                                                                    @if($order->method != "Cash On Delivery")
                                                                        @if($order->method=="RazorPay")
                                                                            {{$order->method}} Order ID: <p>{{$order->razorpay_orderid}}</p>
                                                                        @endif
                                                                        {{$order->method}} Transaction ID: <p id="ttn">{{$order->razorpay_orderid}}</p><a id="tid" style="cursor: pointer;">Edit Transaction ID</a> <a style="display: none; cursor: pointer;" id="tc" >Cancel</a>
                                                                        <form id="tform">
...
{{$order->shipping_state == null ? $order->shipping_state : $order->shipping_state}},{{$order->shipping_country == null ? $order->shipping_country : $order->shipping_country}}
                                                                        </address>
                                                                    @else
                                                            ...
                                                            <div class="row">
                                                                <div class="col-md-6">
                                                                    ...
                                                                    <address>
                                                                        ...
                                                                        Phone: {{$order->customer_phone}}<br>
                                                                        Address: {{$order->customer_address}}<br>
                                                                        {{$order->customer_city}}-{{$order->customer_zip}}, {{$order->customer_state}}, {{$order->customer_country}}
                                                                    </address>
                                                                </div>
                                                                <div class="col-md-6">
                                                                    <h5>Payment Information</h5>
                                                                    <p>Paid Amount: {{$order->currency_sign}}{{ number_format($order->pay_amount * $order->currency_value , 2) }}</p>
																	<p>Shipping Amount: {{$order->currency_sign}}{{ number_format($order->shipping_cost * $order->currency_value , 2) }}</p>
																	<p>Tax: {{$order->currency_sign}}{{ number_format($order->tax * $order->currency_value , 2) }}</p>
                                                                    <p>Payment Method: {{$order->method}}</p>
...
@endif
                            ...
                            <table id="example" class="table">
                                ...
                                @foreach($cart->items as $product)
                                    <tr>
                                            ...
                                            <td>
                                                ...
                                                <a target="_blank" href="{{route('front.page',['id1' => $product['item']['slug']])}}">{{ $product['item']['name']}}</a>
                              ...
                              <a href="javascript:;" data-toggle="modal" data-target="#confirm-delete" class="btn btn-info product-btn" id="license" style="padding: 5px 12px;"><i class="fa fa-eye"></i> View License</a>
                                            ...
                                            <td>{{$product['sku']}}</td>
                                            <td>{{$order->currency_sign}}{{number_format($product['itemprice'] * $order->currency_value,2)}}</td>
